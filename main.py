...
class AyarlarPenceresi:
    def __init__(self, parent, cursor, connection):
        self.parent = parent
        self.cursor = cursor
    ...
    def sifre_degistir(self):
        ...
        self.cursor.execute("SELECT sifre FROM kullanici")
        dogru_sifre = self.cursor.fetchone()[0]

        if eski_sifre == dogru_sifre:
            self.cursor.execute("UPDATE kullanici SET sifre=%s", (yeni_sifre,))
    ...
    def eposta_kaydet(self):
        ...
        self.cursor.execute("UPDATE kullanici SET eposta=%s", (eposta,))
        self.connection.commit()
...
class AjandaUygulamasi:
    def __init__(self, pencere):
        ...
        self.connection = mysql.connector.connect(
            host="31.186.11.120",
            user="san314jandacom_san314jandacom",  # Your MySQL username
            password="v6J::abbdspc",  # Your MySQL password
            database="san314jandacom_db",
            use_pure=False
        )
    ...
    def uygulama_ac(self):
        self.ajanda = Ajanda(self.cursor, self.connection)
    ...
    def ayarlar_ac(self):
        AyarlarPenceresi(self.pencere, self.cursor, self.connection)
    ...
    def eposta_gonder(self):
        ...
        self.cursor.execute("SELECT eposta FROM kullanici")
        eposta_adresi = self.cursor.fetchone()[0]
    ...
    def notlari_eposta_gonder(self, notlar, eposta_adresi):
        ...
        try:
            ...
            mail_from = {
                ...
                "email": "MS_iEG9HH@trial-v69oxl5y7pd4785k.mlsender.net",
            }
            recipients = [
                {
                    "name": "Hocam",
...
class Ajanda:
    ...
    def veritabanini_yukle(self):
        try:
            # Tabloları oluştur
            self.cursor.execute(
                "CREATE TABLE IF NOT EXISTS notlar (tarih TEXT, not_metni TEXT)")
            self.connection.commit()
            ...
            self.cursor.fetchall()  # Sonuçları tüket
        except Exception:
    ...
    def not_ekle(self, tarih, not_metni):
        ...
        self.cursor.execute("INSERT IGNORE INTO notlar (tarih, not_metni) VALUES (%s, %s)", (tarih, not_metni))
        self.connection.commit()

    def not_sil(self, tarih, not_index):
        if tarih in self.notlar:
            if 0 <= not_index < len(self.notlar[tarih]):
                # Silinecek notun metnini al
                silinecek_not_metni = self.notlar[tarih][not_index]
            ...
            messagebox.showerror("Hata", "Belirtilen tarihte not bulunamadı.")
...
def kullanici_ekle(cursor, baglanti):
    ...
    if kayit_sayisi == 0:
        cursor.execute("INSERT INTO kullanici (sifre, eposta) VALUES (?, ?)", ("12345", ""))
        connection.commit()
...
kullanici_ekle(uygulama.cursor, uygulama.connection)
pencere.mainloop()
